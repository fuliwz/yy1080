<template>
  ...
  <aside class="drawer" :class="{open:drawerOpen}" aria-label="分类导航">
    ...
    <div class="drawer-body">
      ...
      <router-link class="drawer-item" to="/history" @click="drawerOpen=false"><i class="bi bi-clock-history"></i>观看历史</router-link>
...
@media(max-width:1100px){.desktop-nav .nav-link:nth-child(n+7){display:none}}@media(max-width:900px){.desktop-nav{display:none}.search-box{width:min(320px,48vw)}}@media(max-width:640px){.header-inner{width:calc(100% - 20px);min-height:60px;gap:9px}.brand{font-size:16px}.menu-btn{width:36px;height:36px}.search-box{width:auto;flex:1;min-width:0}.search-btn{font-size:11px;padding:0 9px}.header-links-inner{width:calc(100% - 20px);min-height:40px;display:flex;flex-wrap:wrap;align-items:center;justify-content:flex-start;gap:8px 10px;overflow:visible;white-space:normal;padding:8px 0}.links-title{flex:0 0 100%;line-height:18px}.header-links a{box-sizing:border-box;flex:0 0 calc((100% - 40px)/5);min-width:0;overflow:hidden;text-overflow:ellipsis;text-align:center;line-height:20px}}
</style>
