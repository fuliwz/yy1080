  ...
  <main class="site-main home-page">
    <div class="page-shell">
      ...
      <section class="quick-nav" aria-label="快速浏览">
        ...
        <button class="quick-item" type="button" @click="scrollTo('featured')"><i class="bi bi-stars"></i><span>精选</span></button>
      </section>

      <section id="featured" v-if="featuredList.length" class="featured-section content-section">
        <div class="section-bar compact">
          <div><div class="section-kicker">EDITOR'S PICK</div><h2 class="section-heading">精选推荐</h2><div class="section-subtitle">从热门与最新内容中精选</div></div>
        </div>
      ...
      <VideoSection id="day-hot" title="今日热门" subtitle="今天播放热度最高的内容" kicker="TODAY'S HOT" :list="dayHot" :loading="loadingDayHot" />
...
import { getLatestVideos, getHotVideos, getDayHotVideos } from '../api/cms'
...
const latest = ref([]), hot = ref([]), dayHot = ref([]), allLatest = ref([])
...
const featuredList = computed(() => [...hot.value.slice(0, 2), ...latest.value].filter((item, i, arr) => arr.findIndex(v => v.vod_id === item.vod_id) === i).slice(0, 3))
...
async function loadData() {
  ...
  latest.value = results[0].status === 'fulfilled' ? results[0].value?.data?.list || [] : []
  hot.value = results[1].status === 'fulfilled' ? results[1].value?.data?.list || [] : []
  dayHot.value = results[2].status === 'fulfilled' ? results[2].value?.data?.list || [] : []
  allLatest.value = results[3].status === 'fulfilled' ? results[3].value?.data?.list || [] : []
  totalPage.value = Number(results[3].status === 'fulfilled' ? results[3].value?.data?.pagecount || 1 : 1)
